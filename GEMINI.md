...
- **Service Locator + Event駆動のハイブリッドアーキテクチャ（最重要）**: グローバルサービスへのアクセスとイベントベースの通信を組み合わせたハイブリッドアアーキテクチャです。
...
1.  **Service Locator + Event駆動のハイブリッドアーキテクチャ（最重要）**
    ...
    *   **実装は、必ずこのアーキテクチャを基盤とし、ServiceLocatorを通じてグローバルサービスにアクセスし、イベント駆動型通信を活用してコンポーネント間の疎結合な連携を実現してください。**

2.  **イベント駆動型アーキテクチャ (Event-Driven Architecture)**:
...
4.  **ObjectPool最適化パターン (Object Pool Pattern)**:
...
-   `Assets/_Project/Features/Templates`: 対応するゲーム機能のテンプレートファイル。 **テンプレートファイルは必ずここに配置してください**。
-   `Assets/_Project/Features/Templates/Common`: 各テンプレートで使用する共通のファイル。 **テンプレートで使用する共通ファイルは必ずここに配置してください**。
