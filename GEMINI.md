...
-   `Assets/_Project/Docs/Works`: 作業報告フォルダ（YYYYMMDD_HHMM）でフォルダを作成し、`SPEC.md` や `REQUIREMENTS.md` など、各ファイルのスナップショットを保管します。 作業報告は `WORKS_LOG.md` に記録してください。 **作業報告は必ずここに配置してください**。
