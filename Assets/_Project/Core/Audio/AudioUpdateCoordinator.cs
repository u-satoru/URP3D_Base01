...
#pragma warning restore CS0414
        ...
        private void Awake()
        {
            ...
            if (FeatureFlags.UseServiceLocator)
            {
                ...
                {
                    ServiceHelper.Log("[AudioUpdateCoordinator] Registered to ServiceLocator as IAudioUpdateService");
                }
            }
        ...
        private void OnDestroy()
        {
            ...
            if (FeatureFlags.UseServiceLocator)
            {
                ...
                if (FeatureFlags.EnableDebugLogging)
                {
                    ServiceHelper.Log("[AudioUpdateCoordinator] Unregistered from ServiceLocator");
                }
            }
        ...
        public void Initialize()
        {
            ...
            if (FeatureFlags.EnableDebugLogging)
            {
                ServiceHelper.Log("[AudioUpdateCoordinator] Initialization complete");
            }
        }
        ...
        private void InitializeCoordinator()
        {
            ...
            ServiceHelper.Log("<color=green>[AudioUpdateCoordinator]</color> Audio update coordinator initialized");
        }
        ...
        private void FindSystemReferences()
        {
            ...
            ServiceHelper.Log($"<color=green>[AudioUpdateCoordinator]</color> Found {GetFoundSystemsCount()} audio systems");
        }
        ...
        public void RegisterUpdatable(IAudioUpdatable updatable)
        {
            if (updatable != null)
            {
                ...
                if (FeatureFlags.EnableDebugLogging)
                {
                    ServiceHelper.Log($"[AudioUpdateCoordinator] Registered updatable: {updatable.GetType().Name}");
                }
            }
        }
        ...
        public void UnregisterUpdatable(IAudioUpdatable updatable)
        {
            if (updatable != null)
            {
                ...
                if (FeatureFlags.EnableDebugLogging)
                {
                    ServiceHelper.Log($"[AudioUpdateCoordinator] Unregistered updatable: {updatable.GetType().Name}");
                }
            }
        }
        ...
        public void StartCoordinatedUpdates()
        {
            if (coordinatedUpdateCoroutine == null)
            {
                ...
                ServiceHelper.Log("<color=green>[AudioUpdateCoordinator]</color> Coordinated updates started");
            }
        }
        ...
        public void StopCoordinatedUpdates()
        {
            if (coordinatedUpdateCoroutine != null)
            {
                ...
                ServiceHelper.Log("<color=green>[AudioUpdateCoordinator]</color> Coordinated updates stopped");
            }
        }
        ...
        private void UpdateRegisteredUpdatables(AudioSystemSyncData syncData)
        {
            foreach (var updatable in registeredUpdatables)
            {
                if (updatable != null)
                {
                    ...
                    {
                        ServiceHelper.LogError($"[AudioUpdateCoordinator] Error updating {updatable.GetType().Name}: {ex.Message}");
                    }
                }
            }
        }
        ...
        private void RebuildSpatialCache()
        {
            ...
            ServiceHelper.Log($"<color=green>[AudioUpdateCoordinator]</color> Rebuilt spatial cache: {totalManagedAudioSources} total, {activeAudioSources} active");
        }
        ...
        private AudioSystemSyncData CreateAudioSystemSyncData()
        {
            ...
            if (audioService != null)
            {
                ...
                {
                    ServiceHelper.LogError($"[AudioUpdateCoordinator] Failed to get audio state from service: {ex.Message}");
                }
            }
            else
            {
                ...
                if (audioManager != null)
                {
                    try
                    {
                        ...
                        if (FeatureFlags.EnableDebugLogging)
                        {
                            ServiceHelper.Log("[AudioUpdateCoordinator] Found AudioManager via FindFirstObjectByType");
                        }
                    }
                    catch (System.Exception ex)
                    {
                        ServiceHelper.LogError($"[AudioUpdateCoordinator] Failed to get audio state from AudioManager: {ex.Message}");
                    }
                }
            }
        ...
        private IAudioService GetAudioService()
        {
            if (FeatureFlags.UseServiceLocator)
            {
                ...
                {
                    ServiceHelper.LogError($"[AudioUpdateCoordinator] Failed to get IAudioService from ServiceLocator: {ex.Message}");
                }
            }
            return null;
        }
