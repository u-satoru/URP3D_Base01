# fix_mojibake_with_line_ending_preservation.py
import os
import re
...
output_report_path = r"D:\UnityProjects\URP3D_Base01\mojibake_final_fix_report.txt"
# プロジェクトのルートディレクトリ
...
def apply_corrections_with_line_ending_preservation(corrections):
    ...
    for file_path, line_data in corrections.items():
        try:
            ...
            # --- 2. 内容をデコードし、行に分割（改行コードも保持） ---
            # chardetでエンコーディングを念のため確認
            import chardet
            encoding = chardet.detect(content_bytes)['encoding'] or 'utf-8'
            content_text = content_bytes.decode(encoding)
            lines = content_text.splitlines(True) # Trueで行末の改行コードを保持
            ...
            for line_num, correct_text in line_data.items():
                if 1 <= line_num <= len(lines):
                    ...
                    # 元の改行コードを取得
                    original_ending = re.search(r'(\r\n|\n)$', original_line)
                    newline_char = original_ending.group(0) if original_ending else ''
            ...
            # --- 4. 修正後の内容でファイルをバイナリモードで上書き保存 ---
            # UTF-8(BOM無し)のバイトデータに変換して書き込む
